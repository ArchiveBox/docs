...
> [!NOTE]
> **Open source ArchiveBox does not support setting up *non-admin* users** with more granular read-only permissions. We do offer this feature and more to [paying clients](https://docs.monadical.com/s/archivebox-consulting-services).
...
> This method is mostly useful when testing the API from browser devtools or external apps where CSRF/CORS is not a concern (e.g. `wget`, `curl`).
